...
- Pipeline: instance syncers (`src/arr/*Syncer.ts`) construct feature classes directly with the injected typed client — no `createXSync(arrType)` factory in the production pipeline. **One** instance per feature per instance run; load, diff, persist, and delete all go through that object (`persist` / `persistNaming` on the instance). Do not `new` a second handler to write. `createXSync` factories stay as test-only wrappers (used by unit tests and e2e); `index.ts` must not route persist through them.
