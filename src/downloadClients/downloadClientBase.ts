...
export abstract class BaseDownloadClientSync {
  ...
  public async syncDownloadClients(config: MergedConfigInstance, serverCache: ServerCache): Promise<DownloadClientSyncResult> {
    ...
    // Calculate diff
    const diff = await this.calculateDiff(validClients, serverClients, serverCache);
    ...
    const removed = config.delete_unmanaged_download_clients?.enabled
      ? await this.deleteUnmanagedClients(
          this.filterUnmanagedClients(serverClients, configClients, config.delete_unmanaged_download_clients),
        )
      : 0;
