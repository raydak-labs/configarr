...
| Quality profiles          | A                    | Base (scores/qualities/cutoff). One class per arr: `QualityProfileSonarrSync` (minUpgrade), `QualityProfileRadarrSync` / `QualityProfileWhisparrSync` (language + minUpgrade), `QualityProfileLidarrSync` / `QualityProfileReadarrSync` (neither). Factory in `qualityProfileSyncer.ts`. |
